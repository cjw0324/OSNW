...
int main(int argc, char *argv[])
{
	...
	while(1)
	{
		for(loop = 0; loop < 3; loop++){
                        ...
                        printf("New Client[%d] Connect: %s:%d\n", loop, inet_ntoa(client_addr.sin_addr),htons(client_addr.sin_port));
			...
			if(loop == 0){
				...
				return_data.max_port = htons(client_addr.sin_port);
				return_data.min_port = htons(client_addr.sin_port);
			}
			else if(max < cal_result){
				//printf("max is change %d to %d\n", max, cal_result);
				max = cal_result;
				strcpy(return_data.max_id, inet_ntoa(client_addr.sin_addr));
				return_data.max_port = htons(client_addr.sin_port);
			}
			else if(min > cal_result){
				min = cal_result;
                                strcpy(return_data.min_id, inet_ntoa(client_addr.sin_addr));
                                return_data.min_port = htons(client_addr.sin_port);
			...
			printf("min=%d from %s:%d\n", return_data.mins, return_data.min_id,return_data.min_port);
        		printf("max=%d from %s:%d\n", return_data.maxs, return_data.max_id,return_data.max_port);
		}
		for(loop = 0; loop <3; loop++){
